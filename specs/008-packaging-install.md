...
| 0.2     | 2026-04-18 | Riff (r12f)  | Add `/etc/vpp/startup.d/10-infmon.conf` (VPP plugin disable snippet) to §5.1 file layout and `debian/` source tree per spec 000 alignment. |
...
| `/etc/vpp/startup.d/10-infmon.conf`           | root  | 0644 | Disables plugin autoload by default (see spec 000). Contains `plugin infmon { disable }`. Operators flip `disable` → `enable` on instances where InFMon should run. |
...
- Each package's `postrm` removes only its own conffiles and
  ...
  installed must not delete `backend.conf`). After removing its own
...
├── infmon-backend.conffiles
├── infmon-backend.postinst
├── infmon-backend.postrm
├── 10-infmon.conf               # VPP startup.d snippet (installed to /etc/vpp/startup.d/)
...
- The explicit `infmon-backend.conffiles` is included for clarity and
  compatibility with older compat levels, but is redundant with debhelper
  compat ≥ 12, which automatically marks files under `/etc/` as conffiles.
...
override_dh_auto_install:
	...
	install -Dm0644 debian/10-infmon.conf \
	    $(CURDIR)/debian/tmp/etc/vpp/startup.d/10-infmon.conf
