...
- `flow-rule add <spec>` adds a rule. Spec uses a compact key=value form, e.g.
  `src_ip=10.0.0.0/8 dst_ip=0.0.0.0/0 ip_proto=6 dscp=0 name=egress-monitor`.
...
- `flow-rule list` lists all rules. Columns: `ID`, `NAME`, `FIELDS`,
  ...
  `dscp`, `mirror_src_ip`); `MAX_KEYS` is the maximum number of distinct
  key tuples the rule will track before eviction kicks in; `FLOWS` is the
  current count of live flows under the rule; `EVICTION` is the eviction
  policy (e.g. `lru`, `oldest`). JSON mode emits an array of rule objects.
...
- `flow list <rule-id|name>` lists the active flows under one flow-rule.
  ...
  `src_ip=10.0.0.7 dst_ip=10.0.1.4 ip_proto=6 dscp=0 mirror_src_ip=192.0.2.10`),
...
- `flow show <rule-id|name> <key>` prints one flow's full counters,
  ...
  whole `src_ip=… dst_ip=… ip_proto=… dscp=…` string arrives as one positional).
  ...
  `infmon-cli flow show egress-monitor 'src_ip=10.0.0.7 dst_ip=10.0.1.4 ip_proto=6 dscp=0'`.
