...
`infmonctl` is that entry point. It is a single Rust binary (crate name `infmon-cli`), distributed in
...
- `flow show <rule-id|name> <key>` prints one flow's full counters,
  ...
  `infmonctl flow show https-egress 'src_ip=10.0.0.7 dst_ip=10.0.1.4 ip_proto=6'`.
...
The CLI uses gRPC over the Unix-domain socket at
`/run/infmon/frontend.sock` (per spec 005). From the operator's
perspective, the wire protocol is an implementation detail behind the
verbs above.
...
Unit and integration tests for `infmonctl` live under `src/cli/test/` and run
